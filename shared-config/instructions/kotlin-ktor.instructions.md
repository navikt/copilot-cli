...
Check `build.gradle.kts` for the actual database library before writing queries. Common patterns in Ktor apps:
...
Koin is a common DI framework for Ktor apps. Install the Koin plugin once in `Application.module()` and define dependencies in separate module files:
