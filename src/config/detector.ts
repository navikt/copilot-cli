...
async function detectKotlinStack(repoPath: string, stack: RepoStackInfo): Promise<void> {
    ...
    // Kafka detection
    stack.hasKafka =
        content.includes('org.apache.kafka') ||
        content.includes('spring-kafka') ||
        content.includes('org.springframework.kafka') ||
        content.includes('io.confluent')
    if (stack.hasKafka) {
...
async function detectTypeScriptStack(repoPath: string, stack: RepoStackInfo): Promise<void> {
    ...
    if (allDeps['next']) {
        ...
        stack.bundler = 'Vite'
        // Check if microfrontend (has *-mikrofrontend in name)
